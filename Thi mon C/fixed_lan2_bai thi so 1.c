...
float Division(int n, int arr[]){
    float _div;
    if (arr[n] != 0){
        _div = (float)arr[n-1]/(float)arr[n];
        printf("Calculate Division %d/%d= %.2f", arr[n-1], arr[n], _div);
    }else printf("Calculate Division %d/%d = INFINITY", arr[n-1], arr[n]);
